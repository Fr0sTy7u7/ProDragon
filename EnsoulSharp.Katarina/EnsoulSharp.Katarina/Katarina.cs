namespace EnsoulSharp.Katarina
{
    ...
    internal class Katarina
    {
        ...
        public static void OnLoad()
        {
            ...
            var MenuKat = new Menu("ensoulsharp.katarina", "Nicky.Katarina", true);
            //Combo
        ...
        private static void Combat()
        {
            ...
            {
                if (target != null && GameObjects.Player.CountEnemyHeroesInRange(R.Range - 200) >= hitr)
                {
            ...
            if (R.IsReady() && target.IsValidTarget(R.Range - 200))
            {
        ...
        private static void OnDraw(EventArgs args)
        {
            ...
            {
                Render.Circle.DrawCircle(GameObjects.Player.Position, Q.Range, Color.HotPink);
            }

            if (Draws.DE.Enabled &&  E.IsReady())
            {
                Render.Circle.DrawCircle(GameObjects.Player.Position, E.Range, Color.BlueViolet);
            }
            ...
            if (Draws.DaggerD.Enabled)
            {
                foreach (var daggers in GameObjects.AllGameObjects)
                {
                    if (daggers.Name == "HiddenMinion" && !daggers.IsDead && daggers.IsValid)
                    {
                        ...
                        if (daggers.CountEnemyHeroesInRange(450) == 0)
                        {
                            Render.Circle.DrawCircle(daggers.Position, 355, Color.GreenYellow);
                            Render.Circle.DrawCircle(daggers.Position, 150, Color.Orange);
                        }
                    }
                }
            }
