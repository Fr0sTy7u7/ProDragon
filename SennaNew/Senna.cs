...
namespace Nekko
{
    ...
    internal class Senna
    {
        ...
        private static void OnOrbwalkerAction(object obj, OrbwalkerActionArgs args)
        {
            if (args.Type == OrbwalkerType.BeforeAttack)
            {
                ...
                if (Q.IsReady() && args.Target != null && args.Target.Type == GameObjectType.AIHeroClient)
                {
                    ...
                    {
                        if (ObjectManager.Player.ManaPercent >= 40)
                        {
                            Q.Cast(t);
                        }
                    }
            ...
            if (args.Type == OrbwalkerType.AfterAttack)
            {
                var target = TargetSelector.GetTarget(Q.Range);
                var getPrediction = W.GetPrediction(target);
                if (W.IsReady() && args.Target != null && args.Target.Type == GameObjectType.AIHeroClient)
                {
                    if (Orbwalker.ActiveMode == OrbwalkerMode.Combo)
                    {
                        W.Cast(getPrediction.CastPosition);
        ...
        private static void LastHit()
        {
            ...
            public static void OnInterrupterSpell(AIHeroClient sender, Interrupter.InterruptSpellArgs arg)
            {
