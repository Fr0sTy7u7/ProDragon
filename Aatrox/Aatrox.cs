namespace EnsoulSharp.Aatrox
{
    ...
    internal class Aatrox
    {
        ...
        public static void OnLoad()
        {
            ...
            var MyMenu = new Menu("ensoulsharp.Aatrox", "EnsoulSharp.Aatrox", true);
        ...
        {
            if (!ObjectManager.Player.HasBuff("AatroxLocusOfPower2") || Q.IsCharging)
            {
                return;
            }
        ...
        {
            if (MenuWrapper.SemiKey.W.Active && W.IsReady() && !ObjectManager.Player.HasBuff("AatroxLocusOfPower2") && 
                !Q.IsCharging)
            {
            ...
            if (MenuWrapper.SemiKey.E.Active && E.IsReady() && !ObjectManager.Player.HasBuff("AatroxLocusOfPower2") && 
                !Q.IsCharging)
            {
                ObjectManager.Player.IssueOrder(GameObjectOrder.MoveTo, Game.CursorPos);
            ...
            if (MenuWrapper.Ult.Key.Active && R.IsReady() && !ObjectManager.Player.HasBuff("AatroxLocusOfPower2") &&
                !Q.IsCharging)
            {
                var target = TargetSelector.GetTarget(R.Range);
                if (target != null && target.IsValidTarget(R.Range))
        ...
        private static void Combo()
        {
            ...
            {
                // ult check
                if (!ObjectManager.Player.HasBuff("AatroxLocusOfPower2"))
                {
        ...
        private static void OnGapcloser(AIHeroClient sender, Gapcloser.GapcloserArgs args)
        {
            ...
            if (Q.IsCharging || ObjectManager.Player.HasBuff("AatroxLocusOfPower2"))
            {
                return;
            }
