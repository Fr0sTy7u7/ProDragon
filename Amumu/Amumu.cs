namespace EnsoulSharp.Amumu
{
    ...
    internal class Amumu
    {
        ...
        public static void OnLoad()
        {
            ...
            Q.SetSkillshot(0.25f, 90, 2000, true, SkillshotType.Line);
            ...
            var MyMenu = new Menu("ensoulsharp.Amumu", "EnsoulSharp.Amumu", true);
            ...
            lane.Add(MenuWrapper.LaneClear.EH);
        ...
        {
            if (!ObjectManager.Player.HasBuff("AmumuLocusOfPower2") || Q.IsCharging)
            {
                return;
            }
        ...
        {
            if (MenuWrapper.SemiKey.W.Active && W.IsReady() && !ObjectManager.Player.HasBuff("AmumuLocusOfPower2") && 
                !Q.IsCharging)
            {
            ...
            if (MenuWrapper.SemiKey.E.Active && E.IsReady() && !ObjectManager.Player.HasBuff("AmumuLocusOfPower2") && 
                !Q.IsCharging)
            {
                ObjectManager.Player.IssueOrder(GameObjectOrder.MoveTo, Game.CursorPos);
            ...
            if (MenuWrapper.Ult.Key.Active && R.IsReady() && !ObjectManager.Player.HasBuff("AmumuLocusOfPower2") &&
                !Q.IsCharging)
            {
                var target = TargetSelector.GetTarget(R.Range);
                if (target != null && target.IsValidTarget(R.Range))
        ...
        private static void Combo()
        {
            ...
            {
                // ult check
                if (!ObjectManager.Player.HasBuff("AmumuLocusOfPower2"))
                {
            ...
            if (!Q.IsCharging)
            {
                    ...
                    ObjectManager.Player.ManaPercent >= MenuWrapper.Harass.Mana.Value)
                {
                        ...
                        if (MenuWrapper.LaneClear.E.Enabled && E.IsReady())
                    {
                        ...
                        if (minions.Any())
                        {
                            var wFarmLocation = E.GetCircularFarmLocation(minions);
                            if (wFarmLocation.Position.IsValid() &&
                                wFarmLocation.MinionsHit >= MenuWrapper.LaneClear.EH.Value)
                            {
                                E.Cast(wFarmLocation.Position);
                                return;
                            }
                        }
                    }
        ...
        private static void OnGapcloser(AIHeroClient sender, Gapcloser.GapcloserArgs args)
        {
            ...
            if (Q.IsCharging || ObjectManager.Player.HasBuff("AmumuLocusOfPower2"))
            {
                return;
            }
